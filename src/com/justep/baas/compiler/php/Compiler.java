...
public class Compiler extends BaseCompiler{
	...
	@Override
	protected PHPCompileContext createContext(String modelDir, String outputDir, boolean outSource) {
		...
		PHPCompileContext context = new PHPCompileContext(modelDir, outputDir, outSource, isOutJar, baasServerLibDir, jarName);		
	...
	protected void compileDir(String sourceDir, String targetDir) throws IOException{
		...
		if (!fSourceDir.exists())
			return;
		if (sourceDir.endsWith(".svn") || sourceDir.endsWith("out") 
				|| sourceDir.endsWith("bin") || sourceDir.endsWith("classes") || sourceDir.endsWith(".settings"))
		{
		...
		for (int i = 0; i < file.length; i++) {
			if (file[i].isFile()) {
				...
				if(!name.endsWith(".classpath") && !name.endsWith(".project")){
					...
					if(name.endsWith(PHPCompileContext.SERVICE_FILE_EXT)){
	...
	private void createServiceFile(Service service,String serviceCode){
		final String outputFileExtensionName = ".php";
		final String outputFileType = "PHP";
	...
	private void compileFile(String file){
		...
		if(null!=actions && actions.size()>0){//没有action的service不生成代码
			...
			for(Action action : actions){
				sb.append(createActionCodeForPHP(action));
			}
			...
			for(String item: CustomActionList)
			{
				//System.out.println("require_once(dirname(__FILE__).'/" + item.replace("\\", "/") + ".php');\r\n");
				referenceStr.append("require_once(APP_ROOT.'/" + item.replace("\\", "/") + ".php');\r\n");
			}			
			
			//System.out.println(referenceStr.toString());
			
			String packageName = Utils.isEmptyString(service.getPackageName()) ? ((PHPCompileContext)getContext()).PackageName : service.getPackageName();			
			String serviceCode = createServiceCodeForPHP(packageName, referenceStr.toString(), service.getRunTimeClassName(), Str2ByteStrForPHP(dbCfg.toString()), sb.toString());
